...
usage() {
  cat <<'USAGE'
Usage: install/install_codex_user.sh [--dry-run]

Install Corfu for Codex in user scope at:
  ~/.agents/skills/corfu
...
if [ "$DRY_RUN" -eq 1 ]; then
  echo "Dry run: would install Corfu $VERSION for Codex user scope at: $DEST"
  exit 0
fi
...
echo "Installed Corfu $VERSION for Codex user scope at: $DEST"
