...
usage() {
  cat <<'USAGE'
Usage: install/install_codex_repo.sh [--dry-run] [repo-path]

Install Corfu for Codex into a repository at:
  <repo-path>/.agents/skills/corfu
...
[ -d "$SOURCE" ] || { echo "ERROR: Corfu Codex source directory not found: $SOURCE" >&2; exit 1; }
...
DEST="$TARGET_REPO/.agents/skills/corfu"
VERSION="unknown"
[ -f "$VERSION_FILE" ] && VERSION="$(tr -d '[:space:]' < "$VERSION_FILE")"

if [ "$DRY_RUN" -eq 1 ]; then
  echo "Dry run: would install Corfu $VERSION for Codex at: $DEST"
  exit 0
fi
