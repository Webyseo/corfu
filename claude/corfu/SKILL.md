...
argument-hint: "[optional: ship | stop | pr | blocked | release | product | roadmap | objective]"
...
Use this live snapshot as evidence. If shell execution is disabled or a command fails, say so and continue with available evidence.

```!
if [ -x "${CLAUDE_SKILL_DIR}/scripts/corfu_snapshot.sh" ]; then
  "${CLAUDE_SKILL_DIR}/scripts/corfu_snapshot.sh"
else
  echo "Corfu snapshot script not found."
